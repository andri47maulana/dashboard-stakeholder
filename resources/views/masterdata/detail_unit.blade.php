...
<div class="card shadow mb-4">
    <div class="container">
        ...
        @if($kebunJsons->count())
            <button class="btn btn-success mb-3 lihat-mapall" data-bs-toggle="modal" data-bs-target="#mapModalAll">
            ...
            <div class="card mb-3">
                <div class="card-body">
                    ...
                    <p><strong>Tile URL:</strong>
                        <a href="{{ $kebun->decoded['tileurl'] }}" target="_blank">
                            {{ $kebun->decoded['tileurl'] }}
                        </a>
                    </p>
...
// Fungsi ambil warna selanjutnya
function getNextColor(index){ return colors[index % colors.length]; }
...
$(document).on("click", ".lihat-map", function() {
    ...
    setTimeout(() => {
        ...
        var kebunColor = getNextColor(colorIndex++);
        L.vectorGrid.protobuf(json.tileurl, {
            vectorTileLayerStyles: {
                [json.name]: {
                    fillColor: kebunColor, fillOpacity: 0.5,
                    color: kebunColor, weight: 1
                }
            }
        }).addTo(mapSingle);
...
var mapModal = new bootstrap.Modal(document.getElementById('mapModalAll'));
    mapModal.show();
// mapAllModalEl.addEventListener('shown.bs.modal', function () {
//     // Hapus map lama kalau ada
    if (mapAll) mapAll.remove();

    // Tambahkan delay kecil agar modal sudah fully visible
    setTimeout(() => {
...
$(document).on("click", ".lihat-mapall", function() {
    ...
    setTimeout(() => {
        ...
        kebunJsons.forEach((json, index) => {
            var kebunColor = getNextColor(index);
            L.vectorGrid.protobuf(json.tileurl, {
                vectorTileLayerStyles: {
                    [json.name]: {
                        fillColor: kebunColor,
        ...
        mapAll.invalidateSize(); // pastikan map render penuh di modal
    }, 200);
});
</script>
@endsection --}}
...
$(document).on("click", ".lihat-map", function() {
    ...
    setTimeout(() => {
        ...
        L.vectorGrid.protobuf(json.tileurl, {
            ...
            interactive: true
        }).addTo(mapSingle).bindPopup(
            `<b>${json.name}</b><br>Luas: ${json.luas_label || '-'}<br>Sertifikat: ${json.sertifikat || '-'}<br>Status: ${json.status || '-'}`
        );
