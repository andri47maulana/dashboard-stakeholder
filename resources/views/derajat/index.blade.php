    ...
    <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $row->unitx->unit }}</td>
                 @if (Auth::user()->region == 'PTPN I HO')
                <td>{{ $row->unitx->region }}</td>
                @endif
                <td>{{ $row->tahun }}</td>
                <td>{{ $row->prioritas_socmap }}</td>
                <td>{{ $row->derajat_kepuasan }}</td>
                <td>{{ $row->derajat_hubungan }}</td>
                <td>
                        ...
                        <button class="btn btn-sm btn-warning editBtn"
                            ...
                            data-lingkungan="{{ $row->lingkungan }}"
                            data-ekonomi="{{ $row->ekonomi }}"
                            data-pendidikan="{{ $row->pendidikan }}"
                            data-sosial="{{ $row->sosial_kesesjahteraan }}"
                            data-okupasi="{{ $row->okupasi }}"
                            data-kepuasan="{{ $row->kepuasan }}"
                            data-kontribusi="{{ $row->kontribusi }}"
                            data-komunikasi="{{ $row->komunikasi }}"
                            data-kepercayaan="{{ $row->kepercayaan }}"
                            data-keterlibatan="{{ $row->keterlibatan }}"
                            data-skor="{{ $row->skor_socmap }}"
                            data-prioritas="{{ $row->prioritas_socmap }}"
                            data-indeks="{{ $row->indeks_kepuasan }}"
                            data-derajatkepuasan="{{ $row->derajat_kepuasan }}"
                            data-derajathubungan="{{ $row->derajat_hubungan }}"
                            data-deskripsi="{{ $row->deskripsi }}"
                            data-narasi="{{ $row->narasi }}"
                            data-toggle="modal"
                        ...
                        <button class="btn btn-info btn-sm detailBtn" 
                            ...
                            data-prioritas="{{ $row->prioritas_socmap }}"
                            data-indeks="{{ $row->indeks_kepuasan }}"
                            data-derajat="{{ $row->derajat_hubungan }}"
                            data-kepuasan="{{ $row->kepuasan }}"
                            data-kontribusi="{{ $row->kontribusi }}"
                            data-komunikasi="{{ $row->komunikasi }}"
                            data-kepercayaan="{{ $row->kepercayaan }}"
  ...
  <div class="modal-dialog modal-xl">
    <form id="formAdd" class="modal-content">
      ...
      <div class="modal-body">
        <div class="row g-2">
          ...
          <div class="col-md-6">
            ...
            <select name="id_unit" id="id_unit" class="form-control select2" required>
              ...
              @foreach($units as $u)
                <option value="{{ $u->id }}">{{ $u->unit }} - {{ $u->region }}</option>
              @endforeach
            </select>
          </div>
          <div class="col-md-6">
        ...
        <!-- hidden field -->
        <input type="hidden" name="input_date" value="{{ now() }}">
