...
<div class="card shadow mb-4">
    <div class="card-body">
        ...
        <?php $__empty_1 = true; $__currentLoopData = $kebunJsons; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kebun): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div class="card mb-3">
                <div class="card-body">
                    <h5><?php echo e($kebun->decoded['name'] ?? '-'); ?></h5>
                    <p><strong>ID:</strong> <?php echo e($kebun->decoded['id'] ?? '-'); ?></p>
                    <p><strong>Geometry Type:</strong> <?php echo e($kebun->decoded['geometrytype'] ?? '-'); ?></p>
                    <p><strong>Tile URL:</strong>
                        <a href="<?php echo e($kebun->decoded['tileurl']); ?>" target="_blank"><?php echo e($kebun->decoded['tileurl']); ?></a>
                    </p>
                    <p><strong>Bounds:</strong> <?php echo e(json_encode($kebun->decoded['bounds'] ?? '-')); ?></p>

                    <!-- tombol lihat 1 kebun -->
                    <button class="btn btn-primary lihat-map" data-index="<?php echo e($loop->index); ?>">
                        Lihat Map
                    </button>
                    <button class="btn btn-warning btn-edit-polygon" data-id="<?php echo e($kebun->id); ?>" 
                            data-judul="<?php echo e($kebun->decoded['name'] ?? ''); ?>"
                            data-json='<?php echo json_encode($kebun->decoded, 15, 512) ?>'>
                        Edit
                    </button>

                    <form method="POST" action="<?php echo e(route('kebun_json.destroy', $kebun->id)); ?>" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus polygon ini?');">
                        <?php echo csrf_field(); ?>
...
<div class="modal fade" id="addPolygonModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="polygonForm" method="POST" action="<?php echo e(route('kebun_json.store')); ?>">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="unit_id" value="<?php echo e($unit->id); ?>">
            
            <div class="mb-3">
                <label class="form-label">Region</label>
                <input type="text" class="form-control" value="<?php echo e($unit->region); ?>" readonly>
            </div>

            <div class="mb-3">
                <label class="form-label">Unit</label>
                <input type="text" class="form-control" value="<?php echo e($unit->unit); ?>" readonly>
            </div>
            <div class="mb-3">
...
<script>
    ...
    $(document).on('click', '.btn-edit-polygon', function() {
        ...
        $('#editRegion').val('<?php echo e($unit->region); ?>');
        $('#editUnit').val('<?php echo e($unit->unit); ?>');
        $('#editJudul').val(judul);
        ...
        $('#editPolygonForm').attr('action', '<?php echo e(route("kebun_json.update", ":id")); ?>'.replace(':id', id));
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laragon\www\dashboard-stakeholder\resources\views/masterdata/detail_unit.blade.php ENDPATH**/ ?>